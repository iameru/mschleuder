...
def test_unit_product(test_app):
    ...
    db.session.add(kg)
    db.session.add(st)
    product = Product(name="Kartoffel", unit_id=1, info="Lecker Kartoffel")
    db.session.add(product)
    product = Product(name="Kohlrabi", unit_id=2, info="Super Kohlrabi")
    db.session.add(product)
    product = Product(name="Mangold", unit_id=1, info="Mega Mangold")
    db.session.add(product)
